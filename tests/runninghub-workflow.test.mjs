...
// RunningHub documents server-side seed randomization unless explicitly overridden.
test('fixed mode sends seeds even when they match cloud defaults', () => {
  const raw = { '1': { class_type: 'Sampler', inputs: { seed: 42, noise_seed: 17, steps: 8 } } };
  const runtime = copy(raw), cloud = copy(raw);
  assert.deepEqual(build(raw, runtime, cloud), []);
  assert.deepEqual(build(raw, runtime, cloud, { includeLiterals: true }), [
    { nodeId: '1', fieldName: 'seed', fieldValue: 42 },
    { nodeId: '1', fieldName: 'noise_seed', fieldValue: 17 }
  ]);
});
