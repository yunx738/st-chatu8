...
export function buildRunningHubOverrides(rawValue, runtimeValue, cloudValue, { includeLiterals = false, workflowId = '' } = {}) {
  ...
  for (const [id, node] of Object.entries(runtime)) {
    ...
    for (const [field, value] of Object.entries(node.inputs)) {
      ...
      // RunningHub randomizes seeds unless they are explicitly included in nodeInfoList.
      const explicitSeed = field === 'seed' || field === 'noise_seed';
      // Skip unchanged static defaults, except seeds and explicit runtime inputs.
      if (fixed && !placeholder && !explicitSeed && same(value, originalValue) && same(value, remote.inputs[field])) continue;
